# train_beto_v11.py - PyTorch Backend (Lightning AI Studio)
# BETO V11 Fine-Tuning: Anti-Overfitting, 3-Classes, Full Metrics, and Artifacts Generation
...
FINAL_MODEL_PATH = f"{STUDIO_PATH}/final_model_beto_v11_3_classes"
RESULTS_FILE = f"{STUDIO_PATH}/academic_results_beto_v11.txt"
CHECKPOINT_PATH = f"{STUDIO_PATH}/checkpoint_beto_v11.pt" 
HISTORY_FILE = f"{STUDIO_PATH}/training_history_beto_v11.csv"

# V11 Anti-Overfitting Hyperparameters
TUNING_TRIALS = 3
TUNING_EPOCHS = 2
FINAL_TRAINING_EPOCHS = 20
EARLY_STOPPING_PATIENCE = 6
WEIGHT_DECAY_RATE = 0.05 # Aggressive V11 weight decay
...
def generate_convergence_plot(history_path, best_epoch):
    ...
    ax_main_acc.set_title('BETO V11 Convergence Analysis (3 Classes)', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    plot_path = f"{STUDIO_PATH}/academic_convergence_beto_v11.png"
...
def plot_confusion_matrix(y_true, y_pred):
    ...
    plt.title('Confusion Matrix - BETO V11', fontsize=14, fontweight='bold')
    plt.ylabel('True Label', fontsize=12)
    plt.xlabel('Predicted Label', fontsize=12)
    plt.savefig(f'{STUDIO_PATH}/academic_confusion_matrix_beto_v11.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    print("=" * 80)
    print("FINE-TUNING: BETO V11 (LIGHTNING AI STUDIO)")
    print("=" * 80)
    ...
    print(f"\n[INFO] Phase 3: Final Training (V11 Optimization)")
    
    model = AutoModelForSequenceClassification.from_pretrained(
        MODEL_NAME, num_labels=3, hidden_dropout_prob=best_dropout, attention_probs_dropout_prob=best_dropout
    ).to(DEVICE)

    optimizer = AdamW(model.parameters(), lr=best_lr, weight_decay=WEIGHT_DECAY_RATE)
